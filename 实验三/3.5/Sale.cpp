...
using namespace std;
    ...
    {   
        char ss[81];
        cout<<"Please enter the name of bookstack you want to import (like book.txt):";
        cin>>ss;
        fstream bookStack;
        bookStack.open(ss,ios::in);//读的模式打开文件
		//bookStack.open("book.txt",ios::in);
		if(!bookStack){
    ...
    void Sale::putData()		//在实验结束前，将图书信息写入书库文件中
        {   
            string NAME;
            cout<<"Please enter the name of bookdata";
            cin>>NAME;
            fstream bookStack;
            char a[30];
            strcpy(a,NAME.c_str());
            bookStack.open(a,ios::out);//写的模式打开文件
            int cnt=0;
            string temp = 0;
